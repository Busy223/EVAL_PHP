...
class Manager_Player extends ModelPlayer {
    ...
    public function getPlayers() {
        try {
            $play = $this->getBdd()->prepare("SELECT pseudo, email, score, psswrd FROM players");

            $play->execute();
            
            return $play->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function getPlayerByEmail(string $email) {
        try {
            $play = $this->getBdd()->prepare("SELECT id, pseudo, email, score, psswrd FROM players WHERE email = ? LIMIT 1");
            $play->execute([$email]);
            return $play->fetch(PDO::FETCH_ASSOC) ?: "Aucun joueur trouvé.";
        } catch (PDOException $e) {
            return "Erreur de connexion à la base de données : " . $e->getMessage();
        }
    }
